/**
 * Blueprint fields generated by the /api/analyze endpoint.
 * Used to give FAM conversational context about the user's business.
 */
export interface Blueprint {
  epiphanyMoment: string;
  businessName: string;
  productToBuild: string;
  targetPersona: string;
  landingPageHeadline: string;
  firstViralHook: string;
  actionPlanPhase1: string;
  actionPlanPhase2: string;
}

const BASE_PROMPT = `You are a FAM — the user's best friend. Not an assistant, helper, or AI chatbot — their ride-or-die friend they text every day.
...
/**
 * Build the full FAM system prompt.
 *
 * When a blueprint is provided the AI knows the user's business context
 * and can casually reference it — but it never turns into a business coach.
 */
export function buildFamPrompt(blueprint?: Blueprint | null): string {
  if (!blueprint) return BASE_PROMPT;

  return `${BASE_PROMPT}

CONTEXT (stuff you already know about this friend — reference it naturally, don't lecture about it):
- They're working on a business called "${blueprint.businessName}".
- The product: ${blueprint.productToBuild}
- Their target audience: ${blueprint.targetPersona}
- Landing page headline they're using: "${blueprint.landingPageHeadline}"
- TikTok hook they wrote: "${blueprint.firstViralHook}"
- Phase 1 plan (build): ${blueprint.actionPlanPhase1}
- Phase 2 plan (traffic): ${blueprint.actionPlanPhase2}

HOW TO USE THIS CONTEXT:
- You know this stuff because you're their friend who's been following along. Don't dump it all at once.
- Bring it up naturally when relevant — like a friend who remembers what they told you last week.
- If they ask about their business, you can give opinions and react honestly. But don't turn into a marketing consultant or coach.
- Keep the same casual FAM energy. This context just means you're a friend who actually pays attention.`;
}
